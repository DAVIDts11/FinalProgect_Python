...
def findMinElbowAngle(last10Frames):
    """
    ...
    min = 360
    minIndex = -1
    for index,frameData in enumerate(last10Frames):
...
def findMaxShoulderExtAng(last10Frames, side_to_camera ="right"):
    """
    Loop through last 10 detected frames and find the maximum 
    ...
    # print('        last_10_Frames    ', last10Frames)
    for index,frameData in enumerate(last10Frames):
...
while  cv2.waitKey(1) < 0: # (cap.isOpened()):
    ...
    if(throwHasDetected):
        ...
        if args.throw_type == "upperarm":
            ...
            if minElbowAngle == 0 :
                minElbowAngle,minIndex = findMinElbowAngle(last_10_Frames)
            if(minIndex != -1):
                cv2.line(frame, last_10_Frames[minIndex]["WristPoint"], last_10_Frames[minIndex]["ElbowPoint"], (0, 100, 100), 3)
                cv2.line(frame, last_10_Frames[minIndex]["ShoulderPoint"], last_10_Frames[minIndex]["ElbowPoint"], (0, 100, 100), 3) 
                cv2.putText(frame,f"Min Elbow Angle flexion = {format(minElbowAngle, '.2f')} deg. ",(last_10_Frames[minIndex]["ShoulderPoint"][0]-100, last_10_Frames[minIndex]["ShoulderPoint"][1]-100),font,  0.5, (0, 255, 255),2,cv2.LINE_8)
        else:        
            ...
            if maxExtSholdAngl == 0 :
                maxExtSholdAngl,maxIndex = findMaxShoulderExtAng(last_10_Frames) 
            if(maxIndex !=-1):
                cv2.line(frame, last_10_Frames[maxIndex]["WristPoint"], last_10_Frames[maxIndex]["ElbowPoint"], (50, 150, 200), 3)
                cv2.line(frame, last_10_Frames[maxIndex]["ShoulderPoint"], last_10_Frames[maxIndex]["ElbowPoint"], (50, 150, 200), 3) 
                cv2.putText(frame,f"Shoulder Extansion = {format(maxExtSholdAngl, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-90, last_10_Frames[maxIndex]["ShoulderPoint"][1]-120),font,  0.5, (50, 150, 200),2,cv2.LINE_8)        
                elbowExtAngle = last_10_Frames[maxIndex]["elbowAngle"]
                cv2.putText(frame,f"Elbow Extansion = {format(elbowExtAngle, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-80, last_10_Frames[maxIndex]["ShoulderPoint"][1]-100),font,  0.5, (50, 150, 200),2,cv2.LINE_8)        

        #Draw arm last detected position before the throw was detected ,
        # and put text with shoulder and elbow angle at that time :
        lastIndex = (appendedFrameCounter-1)%40
        cv2.line(frame, last_10_Frames[lastIndex]["WristPoint"], last_10_Frames[lastIndex]["ElbowPoint"], (200, 100, 30), 3)
        cv2.line(frame, last_10_Frames[lastIndex]["ShoulderPoint"], last_10_Frames[lastIndex]["ElbowPoint"], (200, 100, 30), 3) 
        shoulderLastAngle = last_10_Frames[lastIndex]["ShoulderAngle"]
        cv2.putText(frame,f"Shoulder flexsion = {format(shoulderLastAngle, '.2f')} deg. ",(last_10_Frames[lastIndex]["ShoulderPoint"][0]-90, last_10_Frames[lastIndex]["ShoulderPoint"][1]-70),font,  0.5, (200, 100, 30),2,cv2.LINE_8)        
        elbowLastAngle = last_10_Frames[lastIndex]["elbowAngle"]
        cv2.putText(frame,f"Elbow flexsion = {format(elbowLastAngle, '.2f')} deg. ",(last_10_Frames[lastIndex]["ShoulderPoint"][0]-80, last_10_Frames[lastIndex]["ShoulderPoint"][1]-50),font, 0.5,(200, 100, 30),2,cv2.LINE_8)       
    ...
    if(not throwHasDetected):
        ...
        for pair in POSE_PAIRS:
            ...
            #Adding detected arm moution to the last 10 frames (wich been detected ):
            if (shoulderPoint and elbowPoint and wristPoint and hipPoint):
                ...
                if (len(last_10_Frames)<40):
                    last_10_Frames.append({"WristPoint":wristPoint,"ShoulderPoint":shoulderPoint,"ElbowPoint":elbowPoint,"HipPoint":hipPoint,"elbowAngle":elbowAngle,"ShoulderAngle":shoulderAngle})
                    appendedFrameCounter += 1
                else:
                    last_10_Frames[appendedFrameCounter%40] = {"WristPoint":wristPoint,"ShoulderPoint":shoulderPoint,"ElbowPoint":elbowPoint,"HipPoint":hipPoint,"elbowAngle":elbowAngle,"ShoulderAngle":shoulderAngle}
                    appendedFrameCounter += 1
                # print(throwHasDetected ,"  ", center , "  ", radius)
            
            if(elbowPoint and center and center[0]!=0) :
                #if ball has been thrown :
                maxDist = 110 if args.throw_type == "upperarm" else 120
                if (distance(elbowPoint,center)>maxDist and not ballDrawen):
...
# fs.put(contents, filename=args.videoFileName,contentType = 'video/mp4')
