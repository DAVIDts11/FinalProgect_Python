...
while  cv2.waitKey(1) < 0: # (cap.isOpened()):
    ...
    if(throwHasDetected):
        ...
        if args.throw_type == "upperarm":
            ...
            if minElbowAngle == 0 :
                min,minIndex = findMinElbowAngle(last_10_Frames)
            if(minIndex is not -1):
                ...
                cv2.putText(frame,f"Min Elbow Angle flexion = {format(min, '.2f')} deg. ",(last_10_Frames[minIndex]["ShoulderPoint"][0]-100, last_10_Frames[minIndex]["ShoulderPoint"][1]-100),font,  0.5, (0, 255, 255),2,cv2.LINE_8)
        else:        
            ...
            if maxExtSholdAngl == 0 :
                maxAngle,maxIndex = findMaxShoulderExtAng(last_10_Frames) 
            if(maxIndex is not -1):
                ...
                cv2.putText(frame,f"Shoulder Extansion = {format(maxAngle, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-120, last_10_Frames[maxIndex]["ShoulderPoint"][1]-120),font,  0.5, (50, 150, 200),2,cv2.LINE_8)        
                elbowExtAngle = last_10_Frames[maxIndex]["elbowAngle"]
                cv2.putText(frame,f"Elbow Extansion = {format(elbowExtAngle, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-140, last_10_Frames[maxIndex]["ShoulderPoint"][1]-140),font,  0.5, (50, 150, 200),2,cv2.LINE_8)        
        ...
        cv2.putText(frame,f"Shoulder flexsion = {format(shoulderLastAngle, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-90, last_10_Frames[maxIndex]["ShoulderPoint"][1]-90),font,  0.5, (200, 100, 30),2,cv2.LINE_8)        
        elbowLastAngle = last_10_Frames[lastIndex]["elbowAngle"]
        cv2.putText(frame,f"Elbow flexsion = {format(elbowLastAngle, '.2f')} deg. ",(last_10_Frames[maxIndex]["ShoulderPoint"][0]-80, last_10_Frames[maxIndex]["ShoulderPoint"][1]-80),font, 0.5,(200, 100, 30),2,cv2.LINE_8)       
